...
class Asteriod(pygame.sprite.Sprite):
    ...
    def __init__(self) -> None:
        ...
        super().__init__()
        #self._mode = 0  # 0 = flying, 1 = accelerating
        self.image = pygame.image.load(Settings.get_image("BigAsteroid.bmp")).convert()
        #self.imageindex = 0
        #self.image: pygame.surface.Surface = self.images[self.imageindex]
        self.mask = pygame.mask.from_surface(self.image)
        ...
        self.rect.center = Settings.playground.center
        self._timer_acc = Timer(100)
        self._angle = randint(1,360)
        self.speed_x = self.random_speed()
        self.speed_y = self.random_speed()
        self.rand_pos=(randint(0,Settings.playground.width),randint(0,Settings.playground.height))
...
class Game:
    ...
    def __init__(self) -> None:
        ...
        self._ship = Ship()
        self.asteroid=pygame.sprite.Group(Asteriod())
        self._running = True
        for i in range(2):
            self.asteroid.add(Asteriod())
    ...
    def draw(self) -> None:
        ...
        self.asteroid.draw(self._screen)
        pygame.display.flip()
